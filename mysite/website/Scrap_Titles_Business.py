...
def get_articles(soup_object):
    final_list = []
    big_article = soup_object.find('a', class_="hero-v2-big-story__headline-link")
    big_article_url = 'http://www.bloomberg.com' + big_article.get('href')

    big_article_tuple = (big_article_url, big_article.string)
    final_list.append(big_article_tuple)

    list_of_doubles_articles = soup.find_all('a', class_="two-up-story__headline-link")
    for item in list_of_doubles_articles:
        doubles_article_tuple = ('https://www.bloomberg.com' + item.get('href'), item.string)
